...
impl<'ast> Visit<'ast> for ToolExtractor {
    fn visit_item_impl(&mut self, node: &'ast ItemImpl) {
        ...
        let has_tool_router = node
            .attrs
            .iter()
            .any(|attr| attr.path().is_ident("tool_router"));
...
fn extract_tools_from_mcp() -> Vec<(String, String)> {
    ...
    let mcp_content = fs::read_to_string("src/mcp.rs").expect("Failed to read src/mcp.rs");

    // Parse the file into an AST
    let syntax_tree = syn::parse_file(&mcp_content).expect("Failed to parse src/mcp.rs");
...
fn generate_mcp_help_text(tools: Vec<(String, String)>) -> String {
    ...
    for (name, description) in tools {
        help_text.push_str(&format!(
            "  \x1b[1m{name:<max_name_len$}\x1b[0m  {description}\n"
        ));
    }
